...
void Load(int count, unsigned int setindex, unsigned int tag,
  ...
  // This function should simulate cache behavior:
  // 1. Check if the tag exists in the cache set (hit)
  // 2. If not found, find an empty cache line (miss)
  // 3. If no empty line, evict using LRU algorithm (miss + eviction)
  // 4. Update hit_count, miss_count, eviction_count accordingly
}